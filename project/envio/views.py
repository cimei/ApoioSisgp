...
def envia_planos():  
    ...
    if n_enviados != 'erro_credenciais':
        ...
        for p in planos:
            ...
            registra_log_auto(id, 'Retorno API sobre envio do Plano: ' + str(plano_id) + ' - ' + str(retorno_API_msg))
...
def lista_a_enviar():
    ...
    if n_enviados != 'erro_credenciais':
        ...
        # todos os planos executados em com todas as atividades avaliadas
        planos_avaliados = db.session.query(Pactos_de_Trabalho.pactoTrabalhoId,
                                            catdom_1.c.descricao,
                                            Pactos_de_Trabalho.situacaoId,
                                            ativs.c.qtd_ativs,
...
@envio.route('/enviar_planos', methods = ['GET', 'POST'])
@login_required

def enviar_planos():
    """
    +---------------------------------------------------------------------------------------+
    |Envia planos que não tenham sido previamente enviados.                                 |
    |                                                                                       |
    +---------------------------------------------------------------------------------------+
    """
