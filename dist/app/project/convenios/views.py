...
from project.models import DadosSEI, Convenio, Demanda, User, Programa_Interesse, RefSICONV, Empenho,\
                           Desembolso, Pagamento, Chamadas, MSG_Siconv, Proposta, Programa, Coords
from project.convenios.forms import SEIForm, ProgPrefForm, ListaForm
...
def prog_pref_update(cod_prog):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Programa preferencial atualizado!')
...
def cria_Prog_Pref():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Programa preferencial inserido!')
        return redirect(url_for('convenios.lista_programas_pref'))
...
def update_SEI(SEI_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Registro SEI do convênio atualizado!')
...
def cria_SEI(conv):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Registro SEI para convênio criado!')
        return redirect(url_for('convenios.lista_convenios_SICONV',lista='todos',coord='*'))
...
#
    ...
    for e_d in empenho_desembolso:
        ...
        empenho_desembolso_s.append([e_d_s[0],e_d_s[1],e_d_s[2],e_d_s[3],p_emp,e_d_s[5],siafi,p_des])
